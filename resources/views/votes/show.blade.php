...
@section('content')
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4 cote" data-id="{{ $item->id }}">0</td>
                        <td class="px-4">
                            <a 
                            href="{{route('showIntervenant',[$phaseAndSpeaker->slug, 'candidat' => $item->id])}}"
   ...
   <script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        console.log(votes);
        document.querySelectorAll('.cote').forEach(function(cell) {
            const id = cell.getAttribute('data-id');
            if (votes.candidatId == id) {
                cell.textContent =Number(votes.vote1) + Number(votes.vote2)
            }
        });
    });
    </script>
