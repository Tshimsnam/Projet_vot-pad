<x-app-layout>
    ...
    <div class="">
                                ...
                                <div class="w-full">
                                    <div
                                        ...
                                        <div class="pl-2 pr-5 float-left">
                                            <img class="mt-1 w-16 h-16 object-cover border-2 rounded-md"
        ...
        <div id="jurys-content" class="hidden">
            ...
            <div class="flex justify-between items-center pt-3">
                ...
                <a onclick="ajouter(event, '{{ route('jurys.store') }}', '{{ $phase_id }}', '{{ $ponderation_public }}', '{{ $ponderation_prive }}', '{{ $type_vote }}', '@foreach ($phases as $phase)
                    {{ $phase->type_phase }} @endforeach')"
                        ...
                        <div class="w-full">
                            <div
                                ...
                                <div class="flex justify-between pl-3 relative">
                                    <div>
                                        <h3 class="text-xl text-gray-900 whitespace-nowrap dark:text-white capitalize">
                                            {{ $item->noms }}
                                        </h3>
                                        <div class="flex items-center">
                                            <h3 class="text-gray-900 whitespace-nowrap dark:text-gray-300 capitalize">
                                                {{ $item->coupon }}
                                            </h3>
                                            @if ($item->is_use == 0)
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
        ...
        <div id="critere-content" class="hidden">
            ...
            <div class="py-2 ">
                <div class="flex justify-between items-center">
                    <a onclick="creer(event, {{ $phase_id }}, {{ $sommePonderation }})"
                ...
                <div class="bg-white dark:bg-gray-800 shadow">
                    <div class="mx-auto py-3 px-2">
                        <div class="flex justify-between items-center">
                            <h3 class="font-semibold text-gray-800 dark:text-gray-200 leading-tight"
                                style="text-transform: uppercase">
                                Ponderation total : {{ $sommePonderation }}
                            </h3>
                        </div>
                    </div>
                </div>
                            ...
                            <div
                                ...
                                <div class="flex justify-between pl-3 relative">
                                    ...
                                    <div class="float right">
                                        <div
                                            class=" py-1 font-medium text-gray-900 whitespace-nowrap dark:text-white pr-3">
                                            <a onclick="modifier(event, '{{ route('criteres.update', $item->id) }}', '{{ $item->libelle }}', '{{ $item->description }}', '{{ $item->ponderation }}', '{{ $sommePonderation }}')"
    ...
    <script>
        ...
        function modifier(event, url, libelle, description, ponderation, somme_ponderation) {
            event.preventDefault();
            ...
            const inputTotal = document.querySelector('#edit-modal form div div #totalPond');
            inputTotal.setAttribute('value', somme_ponderation);

            const h3Pond = document.querySelector('#edit-modal form div div #titrePond');
            h3Pond.textContent = 'Ponderation totale : ' + somme_ponderation + ' / 100';
        }

        function creer(event, phase_id, somme_ponderation) {
            event.preventDefault();
            const phaseSelect = document.querySelector('#create-modal form div div #phase');
            phaseSelect.setAttribute('value', phase_id);

            const inputTotal = document.querySelector('#create-modal form div div #totalPond');
            inputTotal.setAttribute('value', somme_ponderation);

            const h3Pond = document.querySelector('#create-modal form div div #titrePond');
            h3Pond.textContent = 'Ponderation totale : ' + somme_ponderation + ' / 100';
        }
        ...
        function ajouter(event, url, phaseId, ponderation_public, ponderation_prive, type_vote, type_phase) {
            event.preventDefault();
            ...
            inputPhaseId.setAttribute('value', phaseId);
            const divVote = document.querySelector('#create-modal-jury form #vote')
            const divEntretien = document.querySelector('#create-modal-jury form #entretien')
            ...
            if (type_phase == 'Vote' || type_phase == 'vote') {
                divVote.style.display = 'block';
                divEntretien.style.display = 'none';
                ...
                ajoutPublicCheckbox.checked = true;
                const requiredChamps = form.querySelectorAll('[required]');
                requiredChamps.forEach(field => {
                    field.removeAttribute('required');
                })
                const noms = document.querySelector('#create-modal-jury form div div #noms');
                noms.required = true;
                divVote.style.display = 'none';
                divEntretien.style.display = 'block';
                buttonEdit.style.display = 'none';
                buttonAnnul.style.display = 'none';
            }
        }
